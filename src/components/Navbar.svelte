<script>
	...
	import { CITY_PHOTO } from '$lib/info';
	...
	onMount(() => {
		...
		tl.to('#menu-right', { right: 0, ease: Expo.easeInOut }, '-=.8');
		tl.from(
			'.menu-link',
			{
				y: 100,
...
<nav
	class="fixed top-0 z-10 flex w-full justify-end pr-6 pl-2 pt-4 text-neutral-content md:px-8">
	<!-- LOGO -->
	{#if url !== '/'}
		<div class="flex-1">
			<div
	...
	{#if $page.url.pathname !== '/'}
		<!-- THEME CHANGER -->
		<div class="mt-3 mr-4  flex-none">
			<ThemeSelect />
		</div>
	{/if}

	<!-- HAMBURGER MENU -->
	<div class="mt-2 cursor-pointer" on:click={openMenu}>
		<!-- hamburger icon -->
		<svg
			class="swap-on h-12 w-12 fill-accent hover:fill-primary"
			xmlns="http://www.w3.org/2000/svg"
			width="32"
			height="32"
			viewBox="0 0 512 512"
...
<div
	...
	<Close32
		class="h-16 w-16 rounded-full bg-base-100 lg:h-32 lg:w-32" />
</div>

<!-- <div
	id="hr"
	class="left-[50%] z-30 hidden h-screen w-1 scale-y-0 bg-base-100 lg:fixed lg:inline-block" /> -->
	...
	<div class="menu-links flex flex-col gap-8">
		<div class="menu-link">
			<a
				...
				>Home <span class="text-2xl">01</span></a>
		</div>
		<div class="menu-link">
			<a
				on:click={closeMenu}
				href="/design"
				class="decoration-3 text-4xl italic underline hover:text-accent lg:text-6xl"
				>Design <span class="text-2xl">02</span></a>
		</div>
		...
		<div class="menu-link">
			<a
				...
				href="/development"
				class="decoration-3 text-4xl italic underline hover:text-accent lg:text-6xl"
				>Development <span class="text-2xl">03</span></a>
		</div>
		<div class="menu-link">
			<div
				...
				Say Hi! <span class="text-2xl">04</span>
			</div>
		</div>

		<div class="menu-link flex flex-col gap-4">
			<div class="text-4xl italic lg:text-6xl">
				Follow me <span class="text-2xl">05</span>
			</div>
			<SocialIcons />
		</div>
...
<div
	id="menu-right"
	class="fixed -right-[51%] z-20 hidden h-screen w-[50%] bg-black bg-cover bg-fixed bg-center bg-blend-luminosity lg:inline-block"
